...
module.exports = function(comments){
	...
	buf.push('');
	buf.push(desc.summary.trim().replace(/^/gm, '  '));
	buf.push('');
	tagDescriptions(buf,comment.tags);
	buf.push(desc.body.trim().replace(/^/gm, '  '));
    });
...
function tagDescriptions(buf,tags){
    var decl, desc;
    
    if(!tags || tags.length === 0) return;

    tags.forEach(function(tag){
	if(!(['param','throw','return'].indexOf(tag.type) < 0)){
	    decl = "  " + tag.type;

	    if(tag.types) decl+= " " + tag.types.join('|');

	    desc = " " + (tag.description || "");

	    if(tag.name) buf.push(decl + tag.name + desc);
	    else buf.push(decl + desc);
	}
    });
